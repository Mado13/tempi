defmodule TempiWeb.UserAuth do
  use TempiWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_tempi_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      TempiWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_user(conn, _opts) do
    ...
    user =
      user_token &&
        ...
        |> Tempi.Repo.preload([:account, :worker, :employer])
  ...
  the current_user:
      ...
      defmodule TempiWeb.PageLive do
        use TempiWeb, :live_view

        on_mount {TempiWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{TempiWeb.UserAuth, :ensure_authenticated}] do
